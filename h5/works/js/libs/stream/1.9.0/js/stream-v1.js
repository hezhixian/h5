...
;(function (root, factory) {
	...
	StreamProvider.prototype = {
		...
		renderUI : function(a) {
			...
			//添加支持手机端相机添加图片 2017.6.1 zhang junwei
			this.fileInputField = fCreateContentEle("<input type='file' "+(this.get('useCamera') ? " accept='image/*' ": "")+(this.get('takeVideo') ? " accept='video/*' ": "")+" style='visibility:hidden;width:0px;height:0px;opacity:0;position:absolute;left:-1000px;'>");
		...
		rebindFileField : function() {
			//this.fileInputField = fCreateContentEle("<input type='file' accept='image/*' style='width:0px;height:0px;opacity:0;position:absolute;left:-1000px;'>");
			//添加支持手机端相机添加图片 2017.6.1 zhang junwei
			this.fileInputField = fCreateContentEle("<input type='file' "+(this.get('useCamera') ? " accept='image/*' ": "")+(this.get('takeVideo') ? " accept='video/*' ": "")+" style='visibility:hidden;width:0px;height:0px;opacity:0;position:absolute;left:-1000px;'>");
			this.get("dragAndDropArea") && !this.get("dragAndDropArea").nodeType && this.set("dragAndDropArea", document.getElementById(this.get("dragAndDropArea")));
		...
		setFileFilters : function() {
			...
			/*（注：用了accept这个属性，微信浏览器会抽风，提示：没有可执行的操作；因此只能去掉）*/
	...
	function Main(cfg){
		...
		this.config = {
			...
			//手机端支持调用相机,multipleFiles配置项应为false
			useCamera:cfg.useCamera || false,
			takeVideo:cfg.takeVideo || false,
		};
		Parent.apply(this, arguments);
	}
